...
SELECT
    COUNT(*) AS never_escaped_animals
FROM
    animals
WHERE
    escape_attempts = 0;
...
SELECT
    COUNT(escape_attempts) AS escape_count,
    neutered
FROM
    animals
GROUP BY
...
from
    animals
    JOIN owners ON owners.full_name = 'Melody Pond'
    AND animals.owners_id = owners.owners_id;
...
from
    animals
    JOIN species ON species.name = 'Pokemon'
    AND species.species_id = animals.species_id;
...
from
    ...
    INNER JOIN animals ON vets.vets_id = (
        SELECT
            vets_id
        from
            vets
...
from
    visits
    INNER JOIN vets ON vets.vets_id = visits.vets_id
    AND date_of_visit BETWEEN 'Apr 1 , 2020'
    AND ' Aug 30, 2020'
...
from
    visits FULL
    OUTER JOIN vets ON vets.vets_id = visits.vets_id FULL
    OUTER JOIN animals ON animals.id = visits.id
ORDER BY
    date_of_visit desc
LIMIT
    1;
...
WHERE
    NOT specializations.species_id = animals.species_id;
